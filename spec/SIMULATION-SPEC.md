...
`pctVit = vitórias × 100 / jogos`. Sorteia-se `r` uniforme em `1..90`.
...
O treinador **humano** tem um mecanismo separado e mais rígido — confiança da diretoria, §6.7.
...
- Sorteia `TB ∈ {0,1}` = quem começa com a bola (0 = mandante).
...
- **Prorrogação nunca é simulada.** Empate em mata-mata vai direto para disputa de pênaltis abstrata (§3.10).
...
- Pool = jogadores não lesionados e não suspensos, ordenados por **força ↓, energia ↓**
...
Depois, nesta ordem:
1. **Fora de posição**: `s = round(s × 0.5)` se a posição natural ≠ posição do slot (slot ≤ 0 conta como fora de posição).
2. `s = max(s, 1)`.
3. **Escala por competição** (multiplica `s`):
   - Seleções: jogador do mesmo país da seleção → ×0.65 se reputação < 3; ×0.85 se = 3; ×0.95 se = 4.
   - Internacional 1: reputação do clube < 3 → ×0.75; = 3 → ×0.85; senão, país 29 → ×0.90.
   - Mundial: rep < 3 → ×0.55; = 3 → ×0.75; senão, fora do continente 0 → ×0.90.
   - Liga nacional: rep < 3 → ×0.85; = 3 → ×0.95.
   - Copa Nacional / Estadual: se mandante tem rep < 3 e visitante ≥ 3, **todo o visitante** ×0.80.
4. **Retorna `s / 10`.**
...
| **Goleiro** | slot == 1 | o goleiro | — | nenhum → `0.1`; fora de posição → `round(GK×0.2)` |
...
`p[i] = w[i]·m[i]`, sorteia `u ~ U(0, Σp)` e devolve o primeiro índice cuja soma acumulada supere `u`.
...
clamp ≥ 0.2 ; pesos-base {50, 50} × {wA, wD}
```
...
se algum time é humano:
      0 zagueiros no adversário: wDef = round(wDef × 0.2)
      1 zagueiro:                wDef = round(wDef × 0.4)
...
clamp ≥ 0.2 ; pesos base × {1.0, wDef, wFora}
...
- **Peculiaridade:** quando o tipo é pênalti e há time humano na partida, o gol **não** é somado ao placar — ele vira o pênalti interativo, que decide. Em IA×IA conta normalmente.
...
Modificadores (os últimos sobrescrevem): se já houve > 5 amarelos → limiar ×2; se já houve ≥ 2
vermelhos → limiar do amarelo passa a `2 × limiarVermelho`; se já houve ≥ 1 lesão → passa a
`5 × limiarLesão`. **Na prática, depois da primeira lesão da partida a taxa de cartões despenca.**
...
- **Disputa em IA×IA (não é chute a chute):** `x = rand(2..8)`, `y = rand(2..8)`; se `x ≥ y` o
...
- **Disputa interativa:** **70% fixos por cobrança, sem nenhum atributo**. Melhor-de-5 e morte súbita;
  ordem dos batedores = posição ↓ (atacantes primeiro), força ↓, energia ↓.
...
- Efeitos do mando: **+0,3** no duelo de posse e **+0,3** no duelo de chance (só nos ticks em que o
  ...
  visitante (invertido, ver §3.6c); e a divisão **44%/56%** de cartões/lesões contra o visitante.
...
contador existe mas nunca é incrementado (a linha de faltas é sempre 0×0)**.
...
menos posse −0,8 (ou −0,3), −0,5 se volante. Gols ×+0,9; gols contra ×−1,5; amarelos ×−0,2;
vermelhos ×−0,8; assistências ×+0,4; chutes no alvo ×+0,3. **Defensivos (1–13)**: venceu a conta de
...
1. **Mando invertido na conversão de chutes** (§3.6c): mandante converte ~8,8%, visitante ~11,1%; e o peso "para fora" é sobrescrito em toda partida com mando.
...
5. **Sobrescritas do limiar de cartão**: após 2 vermelhos vira `2 × limiarVermelho`; após 1 lesão vira `5 × limiarLesão` — ambos derrubam drasticamente os cartões no resto do jogo. Há ainda um ramo inalcançável (`> 10 amarelos`).
6. **Força exibida** usa `round(energia/100 × força)` com divisão inteira → só mostra a força real com energia exatamente 100; caso contrário **exibe 0**. (Só display; o motor não usa.)
...
| **Lateral def.** (ex=0) | Des = `round(F×0.8)+rnd(6)`; Fin = `B+rnd(4)`; Pas = `A+rnd(3)`; Tec = `A+rnd(7)`; Arm = `B+rnd(5)`; Vel = `A+B+rnd(6)`; Gol = `1+rnd(4)` |
| **Lateral of.** (ex=1) | Arm = `round(F×0.5)+rnd(5)`; Fin = `A+B+rnd(4)`; Pas = `A+C+rnd(3)`; Tec = `A+C+rnd(7)`; Des = `A+rnd(4)`; Vel = `A+B+rnd(4)` |
| **Zagueiro** | Des = `round(F×0.9)+rnd(2)`; Gol = `1+rnd(7)`; Vel = `A+B+rnd(4)`; Tec = `A+B+rnd(7)`; Pas = `A+B+rnd(3)`; Fin = `B+rnd(6)`; Arm = `A+rnd(5)` |
| **Volante** (ex=0) | Des = `round(F×0.7)+rnd(6)`; Fin = `A+rnd(4)`; Pas = `A+rnd(3)`; Tec = `A+rnd(7)`; Arm = `A+rnd(5)`; Vel = `A+B+rnd(6)` |
| **Meia armador** (ex=1) | Arm = `F+rnd(2)`; Fin = `A+C+rnd(4)`; Pas = `A+B+rnd(3)`; Tec = `A+C+rnd(7)`; Des = `A+rnd(4)`; Vel = `A+C+rnd(4)` |
| **Atacante** | Fin = `round(F×0.8)+rnd(2)`; Gol = `1+rnd(6)`; Vel = `A+C+rnd(4)`; Tec = `A+C+rnd(7)`; Pas = `A+B+rnd(3)`; Des = `B+rnd(6)`; Arm = `B+A+rnd(5)` |
...
escala por país (nível do país do CLUBE):
   nívelPaís ≤ 13: nívelClube ≤5 → ×0.40 ; <10 → ×0.65 ; senão ×0.75
   senão, se nívelClube < 10: <3 → ×0.50 ; <5 → ×0.60 ; senão ×0.70
...
Taxa semanal `r` pela tabela nível-do-clube × idade:
...
**Gol e Desarme recebem ×1,2** ao serem creditados.
...
bônus += rnd(5)
força = es + round(bônus × min(fd,100)/100)
...
exigência = (diasRestantes < 60) ? salário − salário×desconto/100
                                 : salário + salário×prêmio/100
...
salário = (idade < 32) ? núcleo + bônusEstrela
                       : núcleo − (idade−32)×300 + bônusEstrela
bônusEstrela = (estrela ou topMundial) ? força × 250 : 0
piso 500 ; topMundial → ×1,4 ; júnior → ×0,1
opção "salário mensal" ligada → ×4
```
...
estrela: nível ≥22 e nacionalidade europeia → ×3 ; nível ≥21 e europeia → ×2 ; senão ×1,7
topMundial → ×1,6 ; atacante → ×1,3 ; titular → ×1,2
termo de idade (idade mínima 16):
   <20 → (32−idade)×27 · ≤25 → (32−idade)×22 · <32 → (32−idade)×15
   <34 → (34−idade)×10 · ≥34 → −(idade−34)×50
baseNível += termoIdade ; se ≤ 0 → 60
valor = quadrático × baseNível
desconto por temporada de chegada:
   júnior            → ×0,03 × es
   chegou nesta temp. → ×0,18 (limitado ao preço pedido registrado, se houver)
   temporada passada  → ×0,35
   duas atrás         → ×0,65
   mais antigo        → sem desconto
júnior com es = 10 → ×1,5
```
**O valor cresce quadraticamente com a força** — é de longe o termo dominante.
Aferição: força 50, 24 anos, clube nível 20 → `100² × (600+176) = 7,76 M`.
...
| Valor de mercado | ×1,7 (×2 ou ×3 na Europa) | ×1,6 |
| Salário | bônus `força×250` | + ×1,4 |
...
⚠ O 3‑4‑3 usa o slot 18, que **não conta em nenhum agregado** → na prática o ataque é calculado com
...
atacante pesa **1,67×** um defensor dentro do seu próprio agregado. Jogadores além do 5º/5º/3º são
desperdício puro.
...
- **Fora de posição** ⇔ posição natural ≠ posição exigida pelo slot → **×0,5 na nota inteira** do jogador. Penalidade fixa, independente de quão distante é a improvisação.
- **Não-goleiro no gol**: sofre o ×0,5 **e** o agregado de goleiro vira `round(GK×0,2)`. Exemplo: um jogador de linha com 70 de força no gol rende 1,0 contra 7,0 de um goleiro de 70 — colapso de ~86%.
- **Lado errado (direita/esquerda) NÃO tem penalidade nenhuma de força.** Só é preferência na escalação automática.
- Estilo (volante × armador × ponta) também não tem penalidade de força — só preferência de escalação.

## 5.4 Escalação automática

1. Filtra lesionados e suspensos (e, para clube humano, contratos vencidos).
2. **Ordena por força ↓, energia ↓** — e nada mais.
...
| **Batedor de falta/pênalti** | força ↓, primeiro **titular** com característica Finalização; senão primeiro titular não-goleiro | Creditado como autor em gols de pênalti (5%) e falta (3%), se estiver em campo |
...
Quando estaduais estão ligados, clubes **fora do Brasil** recebem ainda um aporte de **3,2 × folha
...
| Copa nacional | reputação 0–5 | `{3,12,15,30}`×3 · `{7,13,20,70}` · `{10,15,25,80}`×2 |
| Estadual | reputação 0–5 | `{3,5,12,20}` · `{3,12,15,30}`×2 · `{5,12,20,50}` · `{10,15,25,70}`×2 |
...
2) escala por competição: estadual/regional ×0,7 · amistoso ×0,4
3) bônus proporcional à capacidade: k = 0,30
   +0,15 fase de grupos/mata-mata · +0,30 continental 1 · +0,30 seleções/eliminatórias
   +0,15 continental 2 / supercopas         →  público += round(capacidadeSetor × k)
4) qualidade do visitante: Δrep = repVisitante − repMandante
   m = {0, 0,05, 0,10, 0,15, 0,20, 0,25}[|Δrep|]
   Δrep > 0 → ×(1+m)   ·   Δrep < 0 → ×(1−m)
5) torcida: × (apoioDaTorcida / 100)          // medidor do treinador, 80 se não houver
6) ruído por divisão (somado)
7) elasticidade de preço (só com preços customizados):
   Δ = preçoRecomendado − seuPreço ;  público += round(0,03 × público) × Δ
   → cada unidade ABAIXO do recomendado dá ~3% de público; cada unidade ACIMA tira ~3%
8) limita cada setor à capacidade
RECEITA = Σ público[i] × preço[i]
9) multiplicador final: seleções ×5 (fase de Copa do Mundo) ou ×3 ; eliminatórias ×2
```
...
- Senão, se ele joga **na Europa em clube de reputação ≥ 4**: comprador rep ≥ 4 ou divisão 1 → aceita;
  comprador rep = 3 → aceita **exigindo salário × 2**; senão recusa.
- Senão: vendedor rep ≥ 3 e comprador rep ≥ 3 → aceita; vendedor rep = 3 e comprador rep ∈ {1,2} → aceita;
...
|---|---|---|---|---|---|
| Sobra, jogador bom e jovem | ×1,50 | ×1,20 | ×1,20 | ×1,20 | ×1,50 |
| Sobra, fraco (<30) ou velho (>35) | ×0,85 | ×0,80 | ×0,80 | ×0,90 | ×0,90 |
| Escasso (2 … mínimo−1) | ×2,00 | ×2,00 | ×2,00 | ×2,00 | ×2,50 |
| Único na posição | ×2,50 | ×2,50 | ×2,50 | ×2,50 | ×3,00 |
...
de um pedido de empréstimo. **A oferta é sempre `valor de mercado × 1,3`**, sem barganha.
...
TETO = round(lanceMínimo × 2,5)              // = 1,25 × valor de mercado
LANCE_IA = round(lanceMínimo × d) + melhorLanceHumano + lanceMínimo
...
Empréstimo com a diretoria em blocos fixos de **500.000**; juros recalculados como `floor(dívida × 3/100)`
...
`custo = Σ (custoPorAssento × assentosAdicionados) + 100.000` (taxa fixa de mobilização), **pago à vista**.
...
1. Nível de escalada do leilão (×0,82) é **código morto** — só 0,45 e 0,62 acontecem.
